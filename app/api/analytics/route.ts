...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [sessions, conversations, sponsoredMatches, ctaClicks, leads, recentResult] = await Promise.all([
    ...
    return json({ summary: { sessions, conversations, sponsoredMatches, ctaClicks, leads }, recent: recentResult.data });
  } catch (error) {
